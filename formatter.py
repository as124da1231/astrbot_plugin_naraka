...
from .constants import TEAMMATE_COUNT, split_subtype
...
def top_heroes(mode_matches, top_n=5):
    ...
    return sorted(counter.items(), key=lambda kv: -kv[1])[:top_n]
...
def teammate_stats(mode_matches, team_map, person_map, team_count):
    ...
    for m in mode_matches:
        ...
        for mate in real_mates:
            ...
            d = _num(_dl_value(mate.get("dataList"), "damage"))
            s = _num(_dl_value(mate.get("dataList"), "shock_count"))
            slot = agg.setdefault(name, {"dmg": [], "shock": []})
            slot["dmg"].append(d)
            slot["shock"].append(s)
...
# =========================== 概览排版 ===========================
def format_overview(player, tier_scores, cur_stats, cur_mode_name,
    ...
    for i, (hero, cnt) in enumerate(heroes, 1):
    ...
    for i, mate in enumerate(mates, 1):
        ...
        lines.append(f"场均伤害： {mate['avg_damage']:.1f}，场均振刀： {mate['avg_shock']:.1f}")
...
def format_detail(player, mode_matches, person_map, mode_name):
    """逐场明细（只当前模式的场）+ 底部场均。"""
    name = player.get("roleName", "未知玩家")
    n = len(mode_matches)
    if n == 0:
        return f"{name} 当前模式暂无对局记录。"

    lines = [f"详细查询  查询id {name}（{mode_name}）", ""]
    total_dmg = total_kill = total_shock = 0.0
    for idx, m in enumerate(mode_matches, 1):
        ...
        shock = _num(_dl_value(pr.get("dataList"), "shock_count")) if isinstance(pr, dict) else 0
        total_dmg += dmg
        total_kill += kill
        total_shock += shock
        rank_str = f"第{rank}".ljust(3)
        lines.append(f"{idx:<2} {rank_str} {hero}　振刀{int(shock)}　伤害{int(dmg)}　{ts}")
    ...
    lines.append("常用英雄(最多五个)： " + "　".join(f"{h} {c}场" for h, c in heroes))
    return "\n".join(lines)
